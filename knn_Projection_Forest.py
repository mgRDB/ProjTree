...
#test = pd.DataFrame(np.random.rand(50,10))
from ucimlrepo import fetch_ucirepo
#Iris Data
# iris = fetch_ucirepo(id=53)
# test = iris.data.features

#stem, flower = build_tree(test, leaf, split)

#field = planter(test, tree, leaf, split)
#print(test.iloc[5])
#flowers = finder(test.iloc[5], field)
#print(flowers)
#near = closest(test.iloc[5],flowers,k)
#print(near)
#exnear = closest(test.iloc[5],test,k)
#print(exnear)
#match = correct(near, exnear)
#print("Accuracy = ", match/k)
##print("Accuracy = ", acc)

#print(test.iloc[5])
#closest = traversal(test.iloc[5], stem, flower)
#print(closest.t)

#print(flower['Size'][0])
#print(flower[flower['Left'].isna()]['Size'].sum())

#Cancer Data
cancer = fetch_ucirepo(id=17)
test = cancer.data.features

#cell, tumor = build_tree(test, leaf, split)
#print(tumor['Size'][0])
#print(tumor[tumor['Left'].isna()]['Size'].sum())
...
#splits = [5, 6, 7]
#ks = [3, 5, 7]
#flowers = acctable(test, trees, leaves, splits, ks)
#print (flowers)
#flowers.to_csv("yeast-accuracy.csv", index = False)
#Ran into can't multiply by non-int of type float in line 83 for two multiple datasets
#Computer Hardware, Forest Fires, Sonar Bouncing, and Student Performance

#print(knn(test, 5, planter(test, tree, leaf, split), k))
#print(normalized_knn(test, 5, normalized_planter(test, tree, leaf, split), k))
            ...
            for split in split_test: 
                for y in y_test: 
                    for percent in percent_test: 
                        ...
                        sum = 0
                        # print("Inner Points Within The Cluster Close To The Mean")
                        for i in range(y):
                            anomaly = non_anom_inner(std1, mean1, mean2)
...
# sns.PairGrid(anom, hue="class", hue_order=["Iris-setosa","Iris-versicolor","Iris-virginica","normal","anomaly"]).map_offdiag(sns.scatterplot).map_diag(sns.histplot).add_legend()
...
# total = 0
# for i in range(5):
#     anomoly = pd.DataFrame([[np.random.uniform(-2,2), np.random.uniform(-2,2), np.random.uniform(-2,2), np.random.uniform(-2,2)]],
#                        columns = ["sepal length", "sepal width", "petal length", "petal width"], index = ["a"])
#     flowers = finder(anomoly.iloc[0], field)
#     c = closest_with_distance(anomoly.iloc[0],flowers,k)["remove"]
#     print("0 Outliers: #", i)
#     print("Am I an Anomaly: ",anomaly_detector(c, threshhold))
#     print("")
#     total += c.mean()
# adist = pd.DataFrame({"Outliers":[],"Distance":[]})
# adist.loc[len(adist.index)] = [0, total/5]

# # 1 Outlier
# total = 0
# for i in range(4):
#     anomoly.iloc[0][i] = (np.random.uniform(0,2)+2)*random.randrange(-1, 2, 2)
#     flowers = finder(anomoly.iloc[0], field)
#     c = closest_with_distance(anomoly.iloc[0],flowers,k)["remove"]
#     print("1 Outlier: #", i)
#     print("Am I an Anomaly: ",anomaly_detector(c, threshhold))
#     print("")
#     total += c.mean()
#     anomoly.iloc[0][i] = np.random.uniform(-2,2)
# adist.loc[len(adist.index)] = [1, total/4]
...
#     for j in range(1,4):
#         k += 1
#         anomoly.iloc[0][j] = (np.random.uniform(0,2)+2)*random.randrange(-1, 2, 2)
#         flowers = finder(anomoly.iloc[0], field)
#         c = closest_with_distance(anomoly.iloc[0],flowers,k)["remove"]
#         print("2 Outliers: #", i)
#         print("Am I an Anomaly: ",anomaly_detector(c, threshhold))
#         print("")
#         total += c.mean()
#         anomoly.iloc[0][j] = np.random.uniform(-2,2)
#     anomoly.iloc[0][i] = np.random.uniform(-2,2)
# adist.loc[len(adist.index)] = [2, total/k]
...
#     total += c.mean()
#     anomoly.iloc[0][i] = (np.random.uniform(0,2)+2)*random.randrange(-1, 2, 2)
# adist.loc[len(adist.index)] = [3, total/4]

# # 4 Outliers
# total = 0
# for i in range(5):
#     anomoly = pd.DataFrame([[(np.random.uniform(0,2)+2)*random.randrange(-1, 2, 2), (np.random.uniform(0,2)+2)*random.randrange(-1, 2, 2), (np.random.uniform(0,2)+2)*random.randrange(-1, 2, 2), (np.random.uniform(0,2)+2)*random.randrange(-1, 2, 2)]],
#                        columns = ["sepal length", "sepal width", "petal length", "petal width"], index = ["a"])
#     flowers = finder(anomoly.iloc[0], field)
#     c = closest_with_distance(anomoly.iloc[0],flowers,k)["remove"]
#     print("4 Outliers: #", i)
...
#     print("")
#     total += c.mean()
# adist.loc[len(adist.index)] = [4, total/5]

# adist.to_csv("anomaly_distance.csv", index = False)